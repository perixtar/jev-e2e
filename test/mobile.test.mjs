...
test('native cases preserve every action, fixture and intermediate milestone; web v1 stays distinct',async()=>{
  ...
  for(const goal of ['Never tap "Delete account"; tap "Cancel".','Do not click "Publish"; tap "Back".','Continue without tapping "Buy now".','Be careful not to tap "Delete account"; tap "Cancel".','Refrain from clicking "Submit order"; tap "Back".'])await assert.rejects(compileNative(`Case: Negative intent\nGoal: ${goal}\nExpect: text "Safe" is visible`,'ios','on',{inputs:{},auth:{}},remote,new AbortController().signal,[]),/Negative native action clauses/);assert.equal(requests,0);
});
...
test('installed app IDs are not mistaken for build paths and recording fails closed',async()=>{
 ...
 try{
  ...
  const screenshot=makeDriver(),screenshotPath=join(directory,'stale.png');screenshot.ready=true;await writeFile(screenshotPath,'OLD SECRET IMAGE');screenshot.connection.call=async(command,args)=>{if(command==='snapshot')return safeState;if(command==='screenshot')return{path:screenshotPath,identifiers:{appBundleId:'com.example.app',deviceId:'sim'}};throw Error(command);};await assert.rejects(screenshot.screenshot(screenshotPath,AbortSignal.timeout(1000)),/fresh local artifact/);await assert.rejects(readFile(screenshotPath),/ENOENT/);await screenshot.close();
  ...
  const wrongPath=makeDriver(),requested=join(directory,'requested.png'),ownedWrong=join(directory,'preview.png');wrongPath.ready=true;wrongPath.connection.call=async(command,args)=>{if(command==='snapshot')return safeState;if(command==='screenshot'){await writeFile(ownedWrong,'PRIVATE PIXELS');return{path:ownedWrong,identifiers:{appBundleId:'com.example.app',deviceId:'sim'}};}throw Error(command);};await assert.rejects(wrongPath.screenshot(requested,AbortSignal.timeout(1000)),/requested fresh local artifact/);for(const path of [requested,ownedWrong])await assert.rejects(readFile(path),/ENOENT/);await wrongPath.close();
