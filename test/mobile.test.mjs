...
test('native cases preserve every action, fixture and intermediate milestone; web v1 stays distinct',async()=>{
  ...
  for(const goal of ['Sign in as alice with correct-horse.','Use correct-horse to sign in as alice.','Authenticate alice / correct-horse.'])await assert.rejects(compileNative(`Case: Login\nGoal: ${goal}\nExpect: text "Welcome" is visible`,'ios','on',{inputs:{},auth:{}},remote,new AbortController().signal,[]),/fixture references for private inputs/);assert.equal(requests,0);
  for(const title of ['Login alice correct-horse','Authenticate alice / correct-horse','Sign in as alice with correct-horse'])await assert.rejects(compileNative(`Case: ${title}\nGoal: Inspect login safely\nExpect: text "Welcome" is visible`,'ios','on',{inputs:{},auth:{}},remote,new AbortController().signal,[]),/fixture references for private inputs/);assert.equal(requests,0);
