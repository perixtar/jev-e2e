...
test('prose compiler rejects reordered actions, exchanged input bindings and early checks',async()=>{
 ...
 for(const goal of ['Tap "Catalog", then open "Settings".','Tap "Catalog", then delete account.']){
  let calls=0;await assert.rejects(compileNative(`Case: Open settings\nGoal: ${goal}\nExpect: text "Settings" is visible`,'ios','on',{inputs:{},auth:{}},provider({fetchImpl:async()=>{calls++;throw Error('No provider call allowed for unbound action');}}),new AbortController().signal,[]),/freeform native action could not be bound safely/);assert.equal(calls,0);
 }
});
