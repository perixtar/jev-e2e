...
test('native cases preserve every action, fixture and intermediate milestone; web v1 stays distinct',async()=>{
  ...
  let requests=0;const remote=provider({fetchImpl:async()=>{requests++;throw Error('Private source reached provider');}});for(const goal of ['Sign in with "alice@example.com" and "correct-horse", then tap "Sign in".','The OTP is 123456, then tap "Verify".','Log in with alice and correct-horse.'])await assert.rejects(compileNative(`Case: Private prose\nGoal: ${goal}\nExpect: text "Welcome" is visible`,'ios','on',{inputs:{},auth:{}},remote,new AbortController().signal,[]),/fixture references for private inputs/);assert.equal(requests,0);
});
